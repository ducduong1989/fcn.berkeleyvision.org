...
def inference(deploy_file, train_model, class_input_images_dir, class_output_dir):    
    ...
    # load net
    net = caffe.Net(deploy_file, train_model, caffe.TEST)    
    ...
    for index, image_path in enumerate(image_paths):  
      ...
      color_map_img = label_to_color_image(label=segmap, dataset=get_pascal_name())       
      # Save prediction image
      prediction_filename = _PREDICTION_FORMAT % index
      pil_image = Image.fromarray(color_map_img.astype(np.uint8))
...
def main():
    parser = argparse.ArgumentParser(description='generate mask files from trained model')
    ...
    tosei_class_output_dir = '/output_medical_data/tosei/segmentation_results_color'
    kcrc_class_input_images_dir = '/input_medical_data/KCRC/PNGImages'
    kcrc_class_output_dir = '/output_medical_data/KCRC/segmentation_results_color'
